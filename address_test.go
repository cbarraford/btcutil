...
func TestAddresses(t *testing.T) {
	tests := []struct {
		...
		// Segwit address tests.
		{
			...
			valid:   true,
			result: coinutil.TstAddressWitnessPubKeyHash(
				0,
				[20]byte{
			...
			f: func() (coinutil.Address, error) {
				pkHash := []byte{
					0x75, 0x1e, 0x76, 0xe8, 0x19, 0x91, 0x96, 0xd4, 0x54, 0x94,
					0x1c, 0x45, 0xd1, 0xb3, 0xa3, 0x23, 0xf1, 0x43, 0x3b, 0xd6}
				return coinutil.NewAddressWitnessPubKeyHash(pkHash, &chaincfg.MainNetParams)
			},
			net: &chaincfg.MainNetParams,
		},
		{
			...
			valid:   true,
			result: coinutil.TstAddressWitnessScriptHash(
				0,
				[32]byte{
			...
			f: func() (coinutil.Address, error) {
				scriptHash := []byte{
					...
					0x6c, 0x98, 0x56, 0x78, 0xcd, 0x4d, 0x27, 0xa1,
					0xb8, 0xc6, 0x32, 0x96, 0x04, 0x90, 0x32, 0x62}
				return coinutil.NewAddressWitnessScriptHash(scriptHash, &chaincfg.MainNetParams)
			},
			net: &chaincfg.MainNetParams,
		},
		{
			...
			f: func() (coinutil.Address, error) {
				pkHash := []byte{
					0x75, 0x1e, 0x76, 0xe8, 0x19, 0x91, 0x96, 0xd4, 0x54, 0x94,
					0x1c, 0x45, 0xd1, 0xb3, 0xa3, 0x23, 0xf1, 0x43, 0x3b, 0xd6}
				return coinutil.NewAddressWitnessPubKeyHash(pkHash, &chaincfg.TestNet3Params)
			},
			net: &chaincfg.TestNet3Params,
		},
		{
			...
			f: func() (coinutil.Address, error) {
				scriptHash := []byte{
					...
					0x6c, 0x98, 0x56, 0x78, 0xcd, 0x4d, 0x27, 0xa1,
					0xb8, 0xc6, 0x32, 0x96, 0x04, 0x90, 0x32, 0x62}
				return coinutil.NewAddressWitnessScriptHash(scriptHash, &chaincfg.TestNet3Params)
			},
			net: &chaincfg.TestNet3Params,
		},
		{
	...
	for _, test := range tests {
		...
		if err == nil {
			...
			case *coinutil.AddressWitnessPubKeyHash:
				saddr = coinutil.TstAddressSegwitSAddr(encoded)
			case *coinutil.AddressWitnessScriptHash:
				saddr = coinutil.TstAddressSegwitSAddr(encoded)
			}
			...
			case *coinutil.AddressWitnessPubKeyHash:
				...
				expVer := test.result.(*coinutil.AddressWitnessPubKeyHash).WitnessVersion()
			...
			case *coinutil.AddressWitnessScriptHash:
				...
				expVer := test.result.(*coinutil.AddressWitnessScriptHash).WitnessVersion()
				if v := a.WitnessVersion(); v != expVer {
					t.Errorf("%v: witness versions do not match:\n%x != \n%x",
						test.name, expVer, v)
					return
