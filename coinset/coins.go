// Copyright (c) 2014-2017 The coinsuite developers
...
func (cs *CoinSet) TotalValue() (value coinutil.Amount) {
...
func satisfiesTargetValue(targetValue, minChange, totalValue coinutil.Amount) bool {
...
type CoinSelector interface {
	CoinSelect(targetValue coinutil.Amount, coins []Coin) (Coins, error)
}
...
type MinIndexCoinSelector struct {
	MaxInputs       int
	MinChangeAmount coinutil.Amount
}

// CoinSelect will attempt to select coins using the algorithm described
// in the MinIndexCoinSelector struct.
func (s MinIndexCoinSelector) CoinSelect(targetValue coinutil.Amount, coins []Coin) (Coins, error) {
	cs := NewCoinSet(nil)
...
func (s MinNumberCoinSelector) CoinSelect(targetValue coinutil.Amount, coins []Coin) (Coins, error) {
...
func (s MaxValueAgeCoinSelector) CoinSelect(targetValue coinutil.Amount, coins []Coin) (Coins, error) {
	sortedCoins := make([]Coin, 0, len(coins))
	sortedCoins = append(sortedCoins, coins...)
...
func (s MinPriorityCoinSelector) CoinSelect(targetValue coinutil.Amount, coins []Coin) (Coins, error) {
...
// coinutil.Tx, a specific outpoint index, and the number of confirmations
...
func (c *SimpleCoin) Value() coinutil.Amount {
	return coinutil.Amount(c.txOut().Value)
}
